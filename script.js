let superheroesJSON =`[
  ...
  // находим разметку, в которую поместим все карточки
  const container = document.querySelector(".superheroes__container");
  // превращаем JSON-данные в массив JS и записываем все в переменную
  const superheroes = JSON.parse(superheroesJSON);
  // записываем шаблон со звездным рейтингом в переменную
  const template = document.getElementById("template");
 
// перебираем данные из массива объектов, создаем разметку и добавляем туда элементы из массива
 superheroes.forEach ((superhero) => { 
    const card = document.createElement("div"); // создаем карточки супергероев
    card.classList.add("superhero__card");

    // создаем уникальный id для каждой карточки на основе имен, записанных в свойствах объектов массива
    // убираем пробелы и делаем заглавные буквы строчными перед созданием id
    card.id = superhero.name.replace(/ /g, "").toLowerCase();
    const cardId = card.id;

    const header = document.createElement("h2");
    header.classList.add("superhero__header");
    header.textContent = superhero.name;

    const image = document.createElement("img");
    image.src = superhero.url;
    image.alt = superhero.name;

    const universe = document.createElement("p");
    universe.classList.add("superhero__universe");
    universe.textContent = `Вселенная: ${superhero.universe}`;

    const alterego = document.createElement("p");
    alterego.classList.add("superhero__alterego");
    alterego.textContent = `Альтер эго: ${superhero.alterego}`;

    const occupation = document.createElement("p");
    occupation.classList.add("superhero__occupation");
    occupation.textContent = `Род деятельности: ${superhero.occupation}`;


    const friends = document.createElement("p");
    friends.classList.add("superhero__friends");
    friends.textContent = `Друзья: ${superhero.friends}`;


    const superpowers = document.createElement("p");
    superpowers.classList.add("superhero__superpowers");
    superpowers.textContent = `Суперсилы: ${superhero.superpowers}`;

    const info = document.createElement("p");
    info.classList.add("superhero__info");
   info.textContent = `Описание: ${superhero.info}`;

    const url = document.createElement("a");
    url.classList.add("superhero__url");
    url.href = superhero.url;
    url.textContent = 'Click to get the picture';

// добавляем в каждую карточку данные о супергероях и шаблоны рейтинга
// добавляем карточки в разметку
    card.append(header,image,universe,alterego,occupation,friends,superpowers,info,url);
    card.append(template.content.cloneNode(true));
    container.append (card);
  });

  // находим все карточки и записываем их в переменную
  const cards = document.querySelectorAll(".superhero__card");

  for (let i = 0; i < cards.length; i++) {
    const card = cards[i];
    const cardId = card.id;
  
    // находим все звезды из рейтинга и записываем в переменную
    const stars = card.querySelectorAll(".rating__star");
  
    // проходим по каждой звезде рейтинга для каждой карточки
    for (let s = 0; s < stars.length; s++) {
      const star = stars[s];
      const index = s;
  
      // создаем уникальный id и имя для каждой звезды из рейтинга
      star.id = `star_${cardId}_${index}`;
      star.name = `rating_${cardId}`;
  
      // сохраняем рейтинг в локальное хранилище
      star.addEventListener("click", (event) => {
        localStorage.setItem(`rating_${cardId}`, index + 1);
        console.log(`${index + 1} звезд(ы)`);
      });
    }
  }